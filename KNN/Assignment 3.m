...
%KNN classifier
K=3;
z_knn=k_nn_classifier(X_train,y_train,K,X_test);
...
err_knn=(1-length(find(y_test==z_knn))/length(y_test));

fprintf('Bayesian classifier y_test error is %.3f%% \n',err_bayesian*100);
fprintf('KNN classifier y_test error is %.3f%% \n',err_knn*100);
